...
def _parse_external_common_args(*, default_since_minutes: int | None = None) -> dict[str, Any]:
    ...
    email_addr, claim_baseline_timestamp = external_api_service.resolve_external_mail_scope(
        email_addr if email_addr else None,
        claim_token,
    )

    baseline_timestamp = claim_baseline_timestamp
    baseline_timestamp_raw = request.args.get("baseline_timestamp", None)
    if baseline_timestamp_raw not in (None, ""):
        try:
            request_baseline_timestamp = int(baseline_timestamp_raw)
        except Exception as exc:
            raise external_api_service.InvalidParamError("baseline_timestamp 参数无效") from exc
        if request_baseline_timestamp < 1:
            raise external_api_service.InvalidParamError("baseline_timestamp 参数无效")
        baseline_timestamp = max(baseline_timestamp or 0, request_baseline_timestamp)
