...
_ALIAS_TAG_ALLOWED_RE = re.compile(r"[^a-z0-9._-]+")
...
def _normalize_alias_tag(tag: str, *, max_len: int = 32) -> str:
    """将外部输入规范化为可用于 +tag 的安全片段。"""
    cleaned = _ALIAS_TAG_ALLOWED_RE.sub("-", str(tag or "").strip().lower())
...
def _generate_random_alias_tag() -> str:
    """生成 5 位随机数字字母别名片段。"""
    return "".join(secrets.choice(_ALIAS_TAG_ALPHABET) for _ in range(ALIAS_TAG_LENGTH))


def build_plus_alias_email(
    primary_email: str,
    *,
    alias_tag: Optional[str] = None,
    alias_context: Optional[str] = None,
) -> str:
    """基于主邮箱生成随机 5 位 +tag 别名地址。"""
    normalized_primary = str(primary_email or "").strip()
    if not normalized_primary or "@" not in normalized_primary:
        raise PoolServiceError("primary_email 必须是有效邮箱", "invalid_primary_email")

    local_part, domain = normalized_primary.rsplit("@", 1)
    # alias_tag / alias_context 仅保留为兼容入参；实际别名统一使用随机 5 位数字字母。
    _ = alias_tag, alias_context
    tag = _generate_random_alias_tag()
    return f"{local_part}+{tag}@{domain}"


def _read_settings_via_conn(conn) -> dict[str, int]:
...
def claim_random(
    *,
    ...
    alias_tag = _normalize_alias_tag(alias_tag or "") or None
    alias_strategy = (str(alias_strategy or "").strip().lower() or None)
    if alias_strategy and alias_strategy not in {"auto", "tag", "context"}:
        raise PoolServiceError("alias_strategy 必须是 auto/tag/context 之一", "invalid_alias_strategy")
    ...
    try:
        ...
        if account is not None:
            alias_email = None
            if use_alias and (account.get("provider") or "").strip() != "cloudflare_temp_mail" and (account.get("account_type") or "").strip() != "temp_mail":
                alias_email = build_plus_alias_email(account["email"], alias_tag=alias_tag or "")
            account["primary_email"] = account["email"]
            account["alias_email"] = alias_email or ""
            if alias_email:
                account["email"] = alias_email
            return account
        ...
        if provider == "cloudflare_temp_mail":
            ...
            try:
                ...
                inserted["primary_email"] = inserted["email"]
                inserted["alias_email"] = ""
                return inserted
            except pool_repo.PoolRepositoryError as e:
...
def _create_cf_mailbox_for_pool(*, email_domain: Optional[str]) -> tuple[str, dict[str, object]]:
...
def _delete_cf_mailbox_nonblocking(*, email: str, meta: dict[str, object]) -> None:
...
def get_claim_context(*, claim_token: str) -> Optional[dict[str, object]]:
    """
