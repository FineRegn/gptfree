...
| `use_alias` | boolean | No | whether to return a dynamic `+tag` alias for long-lived mailboxes; default `false` |
| `alias_tag` | string | No | compatibility field; the current implementation ignores the supplied value and generates a 5-character lowercase alphanumeric tag |
| `alias_strategy` | string | No | compatibility field: `auto` / `tag` / `context`; the current implementation always generates a random alias |
...
- `use_alias=true` only applies to long-lived mailboxes; `cloudflare_temp_mail` / temp-mail accounts always return the original address
- when aliasing is enabled, the alias format is fixed as `local+5 lowercase alphanumeric chars@domain`, for example `user+a1b2c@outlook.com`
- when aliasing is enabled, response `email` is the alias address, `primary_email` is the canonical mailbox, and `alias_email` is the alias for this claim; later mail-reading APIs may receive the alias and resolve it back to the primary mailbox
...
```json
{
  ...
  "data": {
    ...
    "primary_email": "abc123@zerodotsix.top",
    "alias_email": "",
    "email_domain": "zerodotsix.top",
    "claim_token": "clm_xxx",
