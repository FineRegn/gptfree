...
| `use_alias` | boolean | 否 | 是否为长期邮箱返回动态 `+tag` 别名；默认 `false` |
| `alias_tag` | string | 否 | 兼容字段；当前实现忽略传入值，统一生成 5 位随机小写字母/数字别名片段 |
| `alias_strategy` | string | 否 | 兼容字段：`auto` / `tag` / `context`，当前实现统一随机生成别名 |
...
- `use_alias=true` 仅对长期邮箱生效；`cloudflare_temp_mail` / 临时邮箱始终返回原始邮箱
- 启用别名时，别名格式固定为 `主邮箱本地部分+5位随机小写字母数字@domain`，例如 `user+a1b2c@outlook.com`
- 启用别名时，返回的 `email` 为别名邮箱，`primary_email` 为主邮箱，`alias_email` 为本次别名邮箱；后续读信接口可传别名，系统会自动回溯到主邮箱读取
